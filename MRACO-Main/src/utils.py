...
def parse_args():
    parser = argparse.ArgumentParser(description='HGDDI')
    ...
    parser.add_argument("--edge-drop", type=float, default=0.9,
...
def l1_norm(t, axit=1):
    t = t.float()
    norm = th.norm(t, 1, axit, True) + 1e-12  # 使用 L1 范数
    output = th.div(t, norm)
    output[th.isnan(output) | th.isinf(output)] = 0.0
    return output