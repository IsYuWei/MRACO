...
####定义权重矩阵
    ...
    def forward(self, drug_structure, drug_target, drug_enzyme, drug_path,drug_drug, drug_drug_mask):
        ...
        node_feat = th.cat((drug_feat,structure_feat, enzyme_feat, path_feat), dim=0)
        ...
        enzyme_embedding = node_feat[self.num_drug + self.num_structure:self.num_drug +
                                    self.num_structure + self.num_enzyme].to(self.device)
        ...
        other_loss = drug_drug_reconstruct_loss + drug_structure_reconstruct_loss + drug_path_reconstruct_loss + \
                     drug_enzyme_reconstruct_loss
        #other_loss = drug_drug_reconstruct_loss + drug_structure_reconstruct_loss + drug_target_reconstruct_loss + \
        ...
        #药物相互作用重构损失+其他类型关系损失+通过乘以“self.reg__lambda”来调节正则化的强度。
